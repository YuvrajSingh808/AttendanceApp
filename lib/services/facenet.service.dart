...
class FaceNetService {
  ...
  Future<String> predict(String choice) async {
    /// search closer user prediction if exists
    return await _searchResult(this._predictedData, choice);
  }
  ...
  Future<String> _searchResult(List predictedData, String choice) async {
    /// loads 'database' 🙄
    data = await _dataBaseService.getFaceDataList(choice);
    ...
    int i = 0;
    for (String label in data[i].keys) {
      currDist = _euclideanDistance(data[i][label], predictedData);
